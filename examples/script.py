...
def _quantize_with_memory_check(df: pl.DataFrame, factor: float, limit: float):
    ...
    return quantize_pmw_features(df, factor)
    ...
    for idx_x, (x_bound_left, x_bound_right) in enumerate(zip(x_bounds, x_bounds[1:])):
        for idx_y, (y_bound_left, y_bound_right) in tqdm(enumerate(zip(y_bounds, y_bounds[1:])), total=len(y_bounds) - 1):
            ...
            with timing(f"Quantizing"):
                df_result = _quantize_with_memory_check(df, factor, MEMORY_USAGE_LIMIT)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--transform", "-t", default="default", choices=["default", "pd"],
    ...
    transform = PD_transform if args.transform == "pd" else lambda x: x
