...
from pmw_analysis.constants import DIR_PMW_ANALYSIS
...
def main():
    df_k = pl.read_parquet(pathlib.Path(DIR_PMW_ANALYSIS) / "final_k.parquet")
    df_k = create_occurrence_column(df_k)
    df_k, sun_glint_column = retrieve_possible_sun_glint(df_k)
    df_k.write_parquet(pathlib.Path(DIR_PMW_ANALYSIS) / "final_k.parquet")

    dir_no_sun_glint = pathlib.Path(DIR_PMW_ANALYSIS) / "no_sun_glint"
